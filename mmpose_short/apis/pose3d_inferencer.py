...
from .inference_tracking import (_track_by_iou, _track_by_oks)
from .inference_3d import (collate_pose_sequence,
                         convert_keypoint_definition, extract_pose_sequence)
from .registry import INFERENCERS
    ...
    def visualize(self,
        ...
        for single_input, pred in zip(inputs, preds):
            if isinstance(single_input, str):
                img = cv2.imread(single_input, channel_order='rgb')
            elif isinstance(single_input, np.ndarray):
                img = cv2.COLOR_BGR2RGB(single_input)
            else:
